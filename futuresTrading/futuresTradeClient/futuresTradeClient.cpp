...
FuturesTradeClient<mode>::FuturesTradeClient(const FuturesConfigInfo& _configInfo): configInfo(_configInfo), size(_configInfo.size),
requestPusher(context, ZMQ_PUSH), responseSubscriber(context, ZMQ_SUB), feedSubscriber(context, ZMQ_SUB) {
...
template<>
FuturesTradeClient<SIM>::FuturesTradeClient(const FuturesConfigInfo& _configInfo) : configInfo(_configInfo), size(_configInfo.size),
requestPusher(context, ZMQ_PUSH), responseSubscriber(context, ZMQ_SUB), feedSubscriber(context, ZMQ_SUB), symbol(_configInfo.Symbol) {
  LOG_BOOST << configInfo.TraderFrontAddr << " " << configInfo.MdFrontAddr << " size: " << size << endl;
}
